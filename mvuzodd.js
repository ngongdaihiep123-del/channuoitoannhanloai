...
// =============================================================================
// 2. JSON PATCH LOGIC (BẢN V2 - THÔNG MINH HƠN)
// =============================================================================
...
// Hàm hỗ trợ: Đảm bảo path tồn tại (Tự tạo mảng nếu thiếu)
function ensurePathExists(obj, pathArr) {
    let current = obj;
    for (let i = 0; i < pathArr.length; i++) {
        const key = pathArr[i];
        if (current[key] === undefined || current[key] === null) {
            // Nếu key tiếp theo là số (index), tạo mảng. Nếu không, tạo object.
...
function applyOperation(data, op) {
    ...
    // [V2 UPDATE] Tự động tạo cấu trúc cha nếu thiếu (Chỉ áp dụng cho 'add' hoặc 'replace')
    if (op.op === 'add' || op.op === 'replace') {
        ensurePathExists(data, parentPath);
    }

    const parent = getValueAt(data, parentPath);
    if (!parent && op.op !== 'add') return data; // Nếu không phải add/replace mà parent thiếu thì chịu
    ...
    switch (op.op) {
        ...
        // ... (Giữ các case khác như move, copy, test nếu cần)
    }
    return data;
}
...
function initListener() {
    ...
    eventOn('mag_command_parsed_for_zod', (context, commands) => {
        ...
        if (isModified) {
            ...
            console.log("MVU Data Updated (V2)");
        }
    });
    console.log("MVU Zod Listener V2 Ready");
}
initListener();
